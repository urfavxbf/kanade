...
/**
 * Modern Kanade playback timeline with synchronized beat accents.
 *
 * The visual is intentionally built from rounded capsules and a soft
 * center pulse so it shares the same geometric language as the custom
 * Kanade icons. The gradient always follows the current album accent.
 */
public class SynchronizedBeatSeekBar extends PlayerAudioAnalysisSeekBar {
    ...
    public SynchronizedBeatSeekBar(Context context) {
        super(context);
        initializeModernStyle();
    }

    public SynchronizedBeatSeekBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        initializeModernStyle();
    }

    public SynchronizedBeatSeekBar(
            Context context,
            AttributeSet attrs,
            int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float segmentWidth = Math.max(dp(3f), (width - dp(2f) * (SEGMENT_COUNT - 1)) / SEGMENT_COUNT);
        float gap = dp(2f);
        float maxHeight = height * 0.72f;
        float minHeight = Math.max(dp(3f), height * 0.10f);
        float progress = getMax() > 0 ? getProgress() / (float) getMax() : 0f;

        trackPaint.setColor(Color.argb(34, Color.red(accentColor), Color.green(accentColor), Color.blue(accentColor)));
        ...
        for (int i = 0; i < SEGMENT_COUNT; i++) {
            ...
            float shape = (float) (0.35 + 0.65 * Math.sin(position * Math.PI));
            float progressBoost = position <= progress ? 1f : 0.36f;
            float local = MIN_LEVEL + audio * shape * progressBoost;
            local += beatPulse * (0.10f + bassWeight(position) * 0.28f);
            local = clamp(local);

            float barHeight = minHeight + (maxHeight - minHeight) * local;
