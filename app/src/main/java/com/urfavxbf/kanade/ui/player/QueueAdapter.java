...
public class QueueAdapter extends RecyclerView.Adapter<QueueAdapter.QueueViewHolder> {
    ...
    public void moveItem(int from, int to) {
        if (from < 0 || to < 0 || from >= songs.size() || to >= songs.size() || from == to) {
            return;
        }
    ...
    public void finishDrag() {
        ...
        for (AudioFile song : songs) {
            if (song == null || song.getUri() == null || song.getUri().trim().isEmpty()) {
                continue;
            }
    ...
    private void loadArtwork(AudioFile song, ImageView imageView) {
        ...
        artworkExecutor.execute(() -> {
            ...
            imageView.post(() -> {
                ...
                if (uriString.equals(tag) && result != null && !result.isRecycled()) {
    ...
    private Bitmap decodeArtwork(byte[] artwork) {
        ...
        try {
            return BitmapFactory.decodeByteArray(artwork, 0, artwork.length, options);
    ...
    private int createSecondaryColor(int color) {
        return Color.rgb(
                Math.min(255, Color.red(color) + 25),
                Math.min(255, Color.green(color) + 25),
                Math.min(255, Color.blue(color) + 25)
        );
    }
    ...
    private static final class IntentHelper {
        ...
        static void sendQueueOrder(Context context, ArrayList<String> uris, int currentIndex) {
            ...
            try {
                ...
                // Service may no longer be available while the sheet is closing.
            }
        }
    }
}
