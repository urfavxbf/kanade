...
public final class YouTubeClient {
    ...
    public static ArrayList<Track> searchTracks(String query) throws Exception {
        ...
        List<InfoItem> source = extractor.getInitialPage().getItems();
        ...
        for (InfoItem item : source) {
            ...
            if (url.isEmpty() || title.isEmpty() || !seen.add(url)) {
                continue;
            }
    ...
    public static String resolveStreamUrl(String videoId) throws Exception {
        ...
        AudioStream selected = selectBestAudioStream(streams);
        ...
        if (expiresAt <= 0) {
            expiresAt = System.currentTimeMillis() + 240_000L;
        }
    ...
    private static AudioStream selectBestAudioStream(List<AudioStream> streams) {
        ...
        candidates.sort((a, b) -> {
            int bitrateA = a.getAverageBitrate();
