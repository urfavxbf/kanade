...
public class MusicPlayerService extends Service {
    ...
    private void handleCommand(String action, Intent intent) {
        ...
        if (ACTION_PLAY.equals(action)) {
            ...
            if (mediaPlayer != null && currentUri != null && (uri == null || uri.trim().isEmpty() || uri.equals(currentUri))) resumeCurrent();
            else if (uri != null && !uri.trim().isEmpty()) playSong(uri);
    ...
    private void handleCompletion() { if (repeatMode == REPEAT_ONE && currentUri != null) playSong(currentUri); else playNext(); }
    private void setShuffle(boolean enabled) { shuffleEnabled = enabled; shuffleHistory.clear(); sendShuffleState(); sendQueueChanged(); }
    private int getRandomShuffleIndex() { if (queue.size() <= 1) return 0; ArrayList<Integer> candidates = new ArrayList<>(); for (int i = 0; i < queue.size(); i++) if (i != currentIndex && !shuffleHistory.contains(i)) candidates.add(i); if (candidates.isEmpty()) for (int i = 0; i < queue.size(); i++) if (i != currentIndex) candidates.add(i); if (candidates.isEmpty()) return currentIndex; if (currentIndex >= 0 && !shuffleHistory.contains(currentIndex)) shuffleHistory.add(currentIndex); return candidates.get(random.nextInt(candidates.size())); }
    private int getPreviousShuffleIndex() { while (!shuffleHistory.isEmpty()) { int value = shuffleHistory.remove(shuffleHistory.size() - 1); if (value >= 0 && value < queue.size() && value != currentIndex) return value; } return Math.max(0, currentIndex); }
    ...
    private void loadQueue() { try { ArrayList<AudioFile> result = new MusicRepository(getApplicationContext()).getAllSongs(); if (result != null) { queue.clear(); queue.addAll(result); if (currentUri != null) currentIndex = findSongIndex(currentUri); } } catch (Exception ignored) {} sendQueueChanged(); }
    private AudioFile findSongByUri(String uri) { if (uri == null) return null; for (AudioFile song : queue) if (song != null && uri.equals(song.getUri())) return song; try { for (AudioFile song : new MusicRepository(getApplicationContext()).getAllSongs()) if (song != null && uri.equals(song.getUri())) return song; } catch (Exception ignored) {} return null; }
    private int findSongIndex(String uri) { if (uri == null) return -1; for (int i = 0; i < queue.size(); i++) { AudioFile song = queue.get(i); if (song != null && uri.equals(song.getUri())) return i; } return -1; }
    ...
    private void updateMediaMetadata() { if (mediaSession == null || currentIndex < 0 || currentIndex >= queue.size()) return; AudioFile song = queue.get(currentIndex); if (song == null) return; MediaMetadataCompat.Builder builder = new MediaMetadataCompat.Builder().putString(MediaMetadataCompat.METADATA_KEY_TITLE, safeText(song.getTitle(), "Unknown title")).putString(MediaMetadataCompat.METADATA_KEY_ARTIST, safeText(song.getArtist(), "Unknown artist")).putString(MediaMetadataCompat.METADATA_KEY_ALBUM, safeText(song.getAlbum(), "Unknown album")); if (mediaPlayer != null) try { builder.putLong(MediaMetadataCompat.METADATA_KEY_DURATION, mediaPlayer.getDuration()); } catch (Exception ignored) {} Bitmap art = loadAlbumArt(song); if (art != null) { builder.putBitmap(MediaMetadataCompat.METADATA_KEY_ALBUM_ART, art); builder.putBitmap(MediaMetadataCompat.METADATA_KEY_ART, art); } try { mediaSession.setMetadata(builder.build()); } catch (Exception ignored) {} }
    ...
    private Bitmap loadAlbumArt(AudioFile song) { if (song == null || song.getPath() == null || song.getPath().trim().isEmpty()) return null; android.media.MediaMetadataRetriever retriever = new android.media.MediaMetadataRetriever(); try { retriever.setDataSource(song.getPath()); byte[] data = retriever.getEmbeddedPicture(); if (data != null && data.length > 0) return BitmapFactory.decodeByteArray(data, 0, data.length); } catch (Exception ignored) {} finally { try { retriever.release(); } catch (Exception ignored) {} } return null; }
    private void startAudioAnalysis() { if (mediaPlayer == null) return; try { releaseAudioVisualizer(); int sessionId = mediaPlayer.getAudioSessionId(); if (sessionId <= 0) return; audioVisualizer = new Visualizer(sessionId); int[] range = Visualizer.getCaptureSizeRange(); int size = 1024; if (range != null && range.length >= 2) size = Math.max(range[0], Math.min(1024, range[1])); audioVisualizer.setCaptureSize(size); int rate = Math.min(8000000, Math.max(1, Visualizer.getMaxCaptureRate())); audioVisualizer.setDataCaptureListener(new Visualizer.OnDataCaptureListener() { @Override public void onWaveFormDataCapture(Visualizer visualizer, byte[] waveform, int samplingRate) {} @Override public void onFftDataCapture(Visualizer visualizer, byte[] fft, int samplingRate) { processFFTData(fft, samplingRate); } }, rate, false, true); audioVisualizer.setEnabled(true); audioAnalysisRunning = true; resetAudioAnalysisState(); } catch (Exception ignored) { releaseAudioVisualizer(); } }
    private void processFFTData(byte[] fft, int rateMilliHz) { if (!audioAnalysisRunning || fft == null || fft.length < 4) return; try { if (rateMilliHz > 0) audioVisualizerSampleRate = rateMilliHz / 1000; float bass = calculateBassEnergy(fft), energy = calculateOverallEnergy(fft); smoothedBass = smoothValue(smoothedBass, bass, .45f); smoothedEnergy = smoothValue(smoothedEnergy, energy, .32f); energyBaseline = energyBaseline <= 0 ? smoothedEnergy : smoothValue(energyBaseline, smoothedEnergy, .025f); float bassRise = smoothedBass - previousBass, energyRise = smoothedEnergy - energyBaseline; boolean beat = detectBeat(smoothedBass, smoothedEnergy, bassRise, energyRise); float intensity = Math.max(0, Math.min(1, bassRise * 6f + energyRise * 4.5f + smoothedBass * .45f + smoothedEnergy * .3f)); previousBass = smoothedBass; byte[] copy = new byte[fft.length]; System.arraycopy(fft, 0, copy, 0, fft.length); Intent intent = new Intent(ACTION_AUDIO_ANALYSIS).setPackage(getPackageName()).putExtra(EXTRA_FFT, copy).putExtra(EXTRA_BASS, smoothedBass).putExtra(EXTRA_ENERGY, smoothedEnergy).putExtra(EXTRA_BEAT, beat).putExtra(EXTRA_BEAT_INTENSITY, intensity).putExtra(EXTRA_SAMPLE_RATE, audioVisualizerSampleRate); sendBroadcast(intent); } catch (Exception ignored) {} }
    ...
    private float calculateBandEnergy(byte[] fft, float low, float high) { if (fft == null || fft.length < 4) return 0f; int bins = fft.length / 2; float nyquist = Math.max(1f, audioVisualizerSampleRate / 2f); int start = Math.max(1, Math.round(Math.min(1, low / nyquist) * (bins - 1))); int end = Math.min(bins - 1, Math.round(Math.min(1, high / nyquist) * (bins - 1))); if (end <= start) end = Math.min(bins - 1, start + 1); double total = 0d; int count = 0; for (int i = start; i <= end; i++) { int index = i * 2; if (index + 1 >= fft.length) break; float real = fft[index]; float imaginary = fft[index + 1]; total += Math.sqrt(real * real + imaginary * imaginary); count++; } return count == 0 ? 0f : Math.min(1f, (float) (total / count / 128d)); }
    private float smoothValue(float current, float target, float amount) { return current + (target - current) * amount; }
    private boolean detectBeat(float bass, float energy, float bassRise, float energyRise) { long now = System.currentTimeMillis(); if (now - lastBeatTime < MIN_BEAT_INTERVAL_MS) return false; if (bassRise > .045f && energyRise > .008f) { lastBeatTime = now; return true; } return bassRise > .09f && bass > .12f; }
    private void resetAudioAnalysisState() { smoothedEnergy = 0f; energyBaseline = 0f; smoothedBass = 0f; previousBass = 0f; lastBeatTime = 0L; }
    private void releaseAudioVisualizer() { audioAnalysisRunning = false; if (audioVisualizer != null) { try { audioVisualizer.setEnabled(false); } catch (Exception ignored) {} try { audioVisualizer.release(); } catch (Exception ignored) {} audioVisualizer = null; } }

    private void sendPlaybackState(boolean playing) { sendPlaybackState(playing, mediaPlayer == null ? 0 : safePosition(), mediaPlayer == null ? 0 : safeDuration()); }
    private void sendPlaybackState(boolean playing, int position, int duration) { Intent intent = new Intent(ACTION_STATE_CHANGED).setPackage(getPackageName()); intent.putExtra(EXTRA_IS_PLAYING, playing); intent.putExtra(EXTRA_CURRENT_URI, currentUri); intent.putExtra(EXTRA_POSITION, position); intent.putExtra(EXTRA_DURATION, duration); intent.putExtra(EXTRA_QUEUE_INDEX, currentIndex); sendBroadcast(intent); }
    ...
    @Override public void onDestroy() { stopPositionUpdates(); releaseAudioVisualizer(); releasePlayer(); abandonAudioFocus(); if (mediaSession != null) { try { mediaSession.setActive(false); mediaSession.release(); } catch (Exception ignored) {} mediaSession = null; } if (playbackThread != null) { playbackThread.quitSafely(); playbackThread = null; playbackHandler = null; } super.onDestroy(); }
